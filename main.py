...
def main(csv_filename, write_output = False):
    with open(csv_filename, "r") as csv_file:
        ...
        scenario = vals[header_row.index("DIR")]
        ...
        print(",".join(vals))
        print()
        formatted = format_as_markdown(header_row, vals)
        print(formatted)
        # copy to wayland clipboard
        subprocess.run(["echo", formatted], stdout=subprocess.PIPE, universal_newlines=True)
        subprocess.run(["wl-copy"], input=formatted.encode("utf-8"))
...
def format_as_markdown(header_row, vals):
    ...
    throughput_headers = [
        x for x in header_row if len(x.split()) > 1 and x.split()[1].lower() in ["eps", "bps"]
    ]
    ...
    for i in range(len(throughput_headers)):
        ...
        longest = len(throughput_headers[i])
        tt_headers += f"|{throughput_headers[i].rjust(longest)}"
        tt_sep += f'|{"-" * longest}'
        tt_values += f"|{throughput_values[i].ljust(longest)}"
    ...
    for i in range(len(usage_headers)):
        ...
        ut_sep += f'|{"-" * longest}'
        ut_values += f"|{usage_values[i].ljust(longest)}"
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) > 1 and sys.argv[1] == "--from":
        # recursively get all csv files either from the 3rd argument or the current directory
        root_path = sys.argv[2] if len(sys.argv) > 2 else "."
        for root, dirs, files in os.walk(root_path):
            for file in files:
                if file.endswith(".csv"):
                    print(f"\n\nProcessing {os.path.join(root, file)}")
                    main(os.path.join(root, file), False)
